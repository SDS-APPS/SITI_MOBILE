...
class ProgramGuideManager<T> {
    ...
    private fun updateChannelsTimeRange(selectedDate: LocalDate, timeZone: ZoneId) {
        ...
        if (endUtcMillis > startUtcMillis) {
            for (channel in channels) {
                ...
                if (entries.isEmpty()) {
                    entries.add(ProgramGuideSchedule.createGap(startUtcMillis, endUtcMillis, channel.channelNumber, channel.imageUrl ?: ""))
                } else {
                    ...
                    if (lastEntry == null || endUtcMillis > lastEntry.endsAtMillis) {
                        ...
                        entries.add(
                            ProgramGuideSchedule.createGap(
                                lastEntry?.endsAtMillis
                                    ?: startUtcMillis, endUtcMillis, channel.channelNumber, channel.imageUrl ?: ""
                            )
                        )
                        ...
                        entries.add(
                            ProgramGuideSchedule.createGap(
                                ...
                                endUtcMillis,
                                channel.channelNumber, channel.imageUrl ?: ""
                            )
                        )
                    }
                    ...
                    if (firstEntry == null || startUtcMillis < firstEntry.startsAtMillis) {
                        ...
                        entries.add(
                            0, ProgramGuideSchedule.createGap(
                                startUtcMillis, firstEntry?.startsAtMillis
                                    ?: endUtcMillis, channel.channelNumber, channel.imageUrl ?: ""
                            )
                        )
                        ...
                        entries.add(
                            0,
                            ProgramGuideSchedule.createGap(startUtcMillis, firstEntry.endsAtMillis, channel.channelNumber, channel.imageUrl ?: "")
                        )
                    }
    ...
    fun getCurrentProgram(specificChannelId: String? = null): ProgramGuideSchedule<T>? {
        val firstChannel = channels.lastOrNull() ?: return null
